...
export default class BitcoinApiRouter {
  ...
  async _executeWithFallback(primaryMethod, fallbackMethod, methodName) {
    try {
      ...
      // Suppress expected errors (transaction not yet in mempool after broadcast)
      const isExpectedError = error.message?.includes('No such mempool') || 
      ...
      try {
        ...
        // Only log if not an expected "not found" error
        if (!isExpectedError) {
        }
