 ...
 * Handles validation of payload structure and content
 */
...
export class PayloadValidator {
    ...
    static validatePayload(payload) {
        ...
        PayloadValidator._validateWasmBinaries(payload.binaries);
    ...
    /**
     * Validate WASM binaries
     * @private
     */
    static _validateWasmBinaries(binaries) {
        const wasmHashes = Object.keys(binaries);
        if (wasmHashes.length === 0) {
            throw new Error('No WASM binaries found in payload');
        }

        for (const hash of wasmHashes) {
            const wasmBinary = binaries[hash];
